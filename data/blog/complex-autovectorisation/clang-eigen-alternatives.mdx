...
draft: false
summary: "Clang isn't much better than GCC for complex number multiplication, while Eigen is equivalent to GCC but uses slightly different instructions." 
---
...
Aside from the aligned vs non-aligned ```mov``` instructions, the main difference is that Eigen uses ```vmovsldup``` and ```vmovshdup``` instead of ```vpermilps``` for two of the instructions. However, it turns out this choice is insignificant, as the same number of cycles are used:
...
Honestly, don't know what half of them are, but hey, FMA and AVX are both in there and are relevant to what I do.
